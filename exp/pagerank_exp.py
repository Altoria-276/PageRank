    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            return {
                ...
                "memory": (monitor.peak - base_mem) / (1024**2),
                "result": result
            }
        return wrapper
    return decorator 
...
@experiment("3.内存映射加载（基于 mmap + np.fromstring）")
...
def run_experiments():
    def print_table(results, title, show_iters=False):
        print(f"\n{title:-^60}")
        header = f"{'方法':<25} | {'时间(s)':>8} | {'内存(MB)':>9}"
        if show_iters:
            header += " | 迭代次数"
        print(header)
        print("-"*60)
        for res in results:
            line = f"{res['name']:<25} | {res['time']:>8.3f} | {res['memory']:>9.2f}"
            if show_iters:
    ...
    try:
        ...
        del builder_result
        del data
        for pagerank in [pagerank_baseline, pagerank_enhanced, pagerank_block, 
                        pagerank_block_matrix, pagerank_spider_trap]:
            result = pagerank(adj)
            pr_results.append({
                "name": result["name"],
                "time": result["time"],
