...
def experiment(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        return {"name": func.__name__, "time": time.time() - start_time, "memory": memory_usage, "result": result}
...
def run_experiments():
    ...
    for builder, pr_method in zip(builders, pr_methods):
        queue = mp.Queue()
        proc = mp.Process(target=run_combined_experiment, args=(builder, pr_method, load_full, queue))
        proc.start()
        proc.join()
        result = queue.get()
