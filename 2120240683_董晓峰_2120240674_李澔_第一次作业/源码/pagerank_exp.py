...
def experiment(name):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            return {"name": name, "time": time.time() - start_time, "memory": max(0, (end_mem - start_mem) / (1024**2)), "result": result}
...
def run_experiments():
    ...
    try:
        data = io_results[0]["result"]
        print(f"成功加载数据，共{len(data)}条边")
    ...
    matrix_results = [
        build_csc(data),
        build_csc_optimized(data),
        build_csc_matrix_optimized(data),
    ]

    try:
        matrix = matrix_results[0]["result"]
        print(f"矩阵构建完成，维度：{matrix.shape}")
    except Exception as e:
    ...
    print("\n=== PageRank算法测试 ===")
    pr_results = [
        pagerank_baseline(matrix),
        pagerank_enhanced(matrix),
        pagerank_block(matrix),
        pagerank_block_matrix(matrix),
        pagerank_spider_trap(matrix),
    ]
    ...
    def print_table(results, title, show_iters=False):
        ...
        print(f"\n{title:-^60}")
        header = pad("方法", 40) + " | " + pad("时间(s)", 8) + " | " + pad("内存(MB)", 9)
        if show_iters:
            header += " | " + pad("迭代次数", 8)
        print(header)
        print("-" * 60)
        for res in results:
            line = pad(res["name"], 40) + " | " + pad(f"{res['time']:.3f}", 8) + " | " + pad(f"{res['memory']:.2f}", 9)
            if show_iters:
                line += " | " + pad(str(res["result"]["iterations"]), 8)
            print(line)
